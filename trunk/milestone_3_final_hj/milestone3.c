...
int up_to_network(char *packet, size_t length, int arrived_on_link) {
	...
	if(p->length > maxsize || p->length < (size_t )0){
		fprintf(stdout, "up_to_network, p->length = %d, max size = %d\n", (int)(p->length), (int) maxsize);
		return 0;
	}
	...
	if (p->dest == nodeinfo.address) {
		...
		case NL_DATA:
			...
			if (p->seqno == NL_packetexpected(p->src)) {
				...
				RB_save_msg_link(rb_head, rb_tail, p, arrived_on_link);

				if (p->pieceEnd) {
					RB_copy_whole_msg_link(rb_head, rb_tail, p, arrived_on_link);
